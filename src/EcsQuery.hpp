...
namespace fef {
    ...
    struct QueryIterator {
        ...
        QueryIterator(std::vector<BaseEcsPool*>* pools, bool end) : pools(pools) {
            ...
            for (auto pool : *pools) {
                if (auto size = pool->Size; size < min) {
                    min = size;
                    smallestPool = pool;
                }
            }
            ...
            this->ptr = end || smallestPool->Size == 0 ? beginPtr : &smallestPool->Entities[smallestPool->Size - 1]; // if this is "end" iterator or there are no entities, then it must be first entity
        }
